...
(
    s String,
    INDEX idx_s s TYPE text(tokenizer = ngrams(3), preprocessor = lower(s)) GRANULARITY 100000000
)
ENGINE = MergeTree()
ORDER BY tuple();
...
(
    s String,
    INDEX idx_s s TYPE text(tokenizer = splitByNonAlpha, preprocessor = lower(s)) GRANULARITY 100000000
)
ENGINE = MergeTree()
ORDER BY tuple();
