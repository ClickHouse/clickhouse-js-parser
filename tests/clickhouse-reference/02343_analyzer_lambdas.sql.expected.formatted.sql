...
lambda_1 AS lambda_2,

lambda_2 AS lambda_3
...
SELECT
    id,
    test_table.* APPLY(x -> plus(compound_value.*))
FROM test_table;
...
SELECT
    arrayMap(lambda(tuple(x), x + 1), [1, 2, 3]),
    lambda(1);
...
SELECT
    arrayMap(lambda(tuple(x), x + 1), [1, 2, 3]),
    lambda(tuple(x), x + 1),
    1 AS x; -- { serverError BAD_ARGUMENTS }
