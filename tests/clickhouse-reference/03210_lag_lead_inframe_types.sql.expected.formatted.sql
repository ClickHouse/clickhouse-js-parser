SELECT lagInFrame(CAST('2' AS UInt128), 2, number) OVER w
FROM numbers(10)
WINDOW w AS (ORDER BY number ASC);

SELECT leadInFrame(CAST('2' AS UInt128), 2, number) OVER w
FROM numbers(10)
WINDOW w AS (ORDER BY number ASC);

SELECT lagInFrame(CAST('2' AS UInt64), 2, number) OVER w
FROM numbers(10)
WINDOW w AS (ORDER BY number ASC);

SELECT leadInFrame(CAST('2' AS UInt64), 2, number) OVER w
FROM numbers(10)
WINDOW w AS (ORDER BY number ASC);
