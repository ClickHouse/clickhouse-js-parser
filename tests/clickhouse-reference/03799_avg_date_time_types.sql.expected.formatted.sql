...
SELECT avg(d)
FROM (
        SELECT toDate('2020-01-01') AS d
        UNION ALL
        SELECT toDate('2020-01-03') AS d
    );

SELECT toTypeName(avg(d))
FROM (
        SELECT toDate('2020-01-01') AS d
        UNION ALL
        SELECT toDate('2020-01-03') AS d
...
SELECT avg(d)
FROM (
        SELECT toDate32('2020-01-01') AS d
        UNION ALL
        SELECT toDate32('2020-01-03') AS d
    );
...
SELECT avg(dt)
FROM (
        SELECT toDateTime('2020-01-01 00:00:00', 'UTC') AS dt
        UNION ALL
        SELECT toDateTime('2020-01-01 00:00:02', 'UTC') AS dt
    );

SELECT toTypeName(avg(dt))
FROM (
        SELECT toDateTime('2020-01-01 00:00:00', 'UTC') AS dt
    );
...
SELECT avg(dt64)
FROM (
        SELECT toDateTime64('2020-01-01 00:00:00.000', 3, 'UTC') AS dt64
        UNION ALL
        SELECT toDateTime64('2020-01-01 00:00:00.002', 3, 'UTC') AS dt64
    );

SELECT toTypeName(avg(dt64))
FROM (
        SELECT toDateTime64('2020-01-01 00:00:00.000', 3, 'UTC') AS dt64
    );
...
SELECT avg(t)
FROM (
        SELECT toTime('01:00:00') AS t
        UNION ALL
        SELECT toTime('03:00:00') AS t
    );

SELECT toTypeName(avg(t))
FROM (
        SELECT toTime('01:00:00') AS t
    );
...
FROM (
        ...
        UNION ALL
        SELECT toTime64('03:00:00.000', 3) AS t64
    );

SELECT toTypeName(avg(t64))
FROM (
        SELECT toTime64('01:00:00.000', 3) AS t64
    );
...
SELECT avg(d)
FROM (
        SELECT toDate('2020-06-15') AS d
    );

SELECT avg(d32)
FROM (
        SELECT toDate32('2020-06-15') AS d32
    );

SELECT avg(dt)
FROM (
        SELECT toDateTime('2020-06-15 12:30:45', 'UTC') AS dt
    );

SELECT avg(dt64)
FROM (
        SELECT toDateTime64('2020-06-15 12:30:45.500', 3, 'UTC') AS dt64
    );

SELECT avg(t)
FROM (
        SELECT toTime('12:30:45') AS t
    );

SELECT avg(t64)
FROM (
        SELECT toTime64('12:30:45.500', 3) AS t64
    );
...
SELECT avg(dt64)
FROM (
        SELECT toDateTime64('2020-06-15 12:30:45.123', 3, 'UTC') AS dt64
        FROM numbers(5)
    );
...
SELECT avg(t64)
FROM (
        SELECT toTime64('12:30:45.123', 3) AS t64
        FROM numbers(5)
    );
...
FROM (
        SELECT toDateTime('2020-01-01 00:00:00', 'UTC') AS dt
        UNION ALL
        SELECT toDateTime('2020-01-01 00:00:01', 'UTC') AS dt
    );
...
FROM (
        SELECT toDateTime64('2020-01-01 00:00:00.000', 3, 'UTC') AS dt64
        UNION ALL
        SELECT toDateTime64('2020-01-01 00:00:00.001', 3, 'UTC') AS dt64
    );

SELECT avg(t64)
FROM (
        SELECT toTime64('00:00:00.000', 3) AS t64
        UNION ALL
        SELECT toTime64('00:00:00.001', 3) AS t64
    );

SELECT avg(dt64)
FROM (
        SELECT toDateTime64('2020-01-01 00:00:00', 0, 'UTC') AS dt64
        UNION ALL
        SELECT toDateTime64('2020-01-01 00:00:02', 0, 'UTC') AS dt64
    );
...
SELECT avg(dt64)
FROM (
        SELECT toDateTime64('2020-01-01 00:00:00.000000', 6, 'UTC') AS dt64
        UNION ALL
        SELECT toDateTime64('2020-01-01 00:00:00.000002', 6, 'UTC') AS dt64
    );
...
SELECT avg(dt64)
FROM (
        SELECT toDateTime64('2020-01-01 00:00:00.000000000', 9, 'UTC') AS dt64
        UNION ALL
        SELECT toDateTime64('2020-01-01 00:00:00.000000002', 9, 'UTC') AS dt64
    );
...
SELECT avg(t64)
FROM (
        SELECT toTime64('01:00:00', 0) AS t64
        UNION ALL
        SELECT toTime64('03:00:00', 0) AS t64
    );
...
SELECT avg(t64)
FROM (
        SELECT toTime64('01:00:00.000000', 6) AS t64
        UNION ALL
        SELECT toTime64('03:00:00.000000', 6) AS t64
    );
...
SELECT avg(d)
FROM (
        SELECT toDate('1970-01-01') AS d
        UNION ALL
        SELECT toDate('2100-12-31') AS d
    );

SELECT avg(d32)
FROM (
        SELECT toDate32('1900-01-01') AS d32
        UNION ALL
        SELECT toDate32('2100-01-01') AS d32
    );

SELECT avg(dt)
FROM (
        SELECT toDateTime('1970-01-01 00:00:00', 'UTC') AS dt
        UNION ALL
        SELECT toDateTime('2100-01-01 00:00:00', 'UTC') AS dt
    );
...
SELECT avg(dt)
FROM (
        SELECT addSeconds(toDateTime('2020-01-01 00:00:00', 'UTC'), number) AS dt
        FROM numbers(10)
    );

SELECT avg(t)
FROM (
        SELECT CAST(number AS Time) AS t
        FROM numbers(10)
    );
...
SELECT
    d,
    avg(d) OVER (ORDER BY d ASC ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING) AS moving_avg
FROM (
        SELECT addDays(toDate('2020-01-01'), number) AS d
        FROM numbers(5)
    )
...
SELECT avgMerge(state)
FROM (
        SELECT avgState(d) AS state
        FROM (
                SELECT toDate('2020-01-01') AS d
                UNION ALL
                SELECT toDate('2020-01-02') AS d
        ...
        FROM (
                SELECT toDate('2020-01-03') AS d
                UNION ALL
                SELECT toDate('2020-01-04') AS d
            )
    );

SELECT avgMerge(state)
FROM (
        SELECT avgState(dt) AS state
        FROM (
                SELECT toDateTime('2020-01-01 00:00:00', 'UTC') AS dt
                UNION ALL
                SELECT toDateTime('2020-01-01 00:00:02', 'UTC') AS dt
        ...
        SELECT avgState(dt) AS state
        FROM (
                SELECT toDateTime('2020-01-01 00:00:04', 'UTC') AS dt
                UNION ALL
                SELECT toDateTime('2020-01-01 00:00:06', 'UTC') AS dt
            )
    );
...
SELECT
    ...
    toTypeName(avg(d32))
FROM (
        SELECT toDate32('2020-01-01') AS d32
    );
...
SELECT avg(dt64)
FROM (
        SELECT addSeconds(toDateTime64('2020-01-01 00:00:00.000', 3, 'UTC'), number) AS dt64
        FROM numbers(100)
    );
