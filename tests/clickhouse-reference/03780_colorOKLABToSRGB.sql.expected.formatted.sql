...
WITH colorOKLABToSRGB((0.4466, 0.0991, 0.0699), 1.) AS t
...
WITH colorOKLABToSRGB((0.4466, 0.0991, 0.0699), 3.) AS t
...
WITH colorOKLABToSRGB((0.591, 0.091, -0.0796), 0) AS t

SELECT tuple(round((t).1, 6), round((t).2, 6), round((t).3, 6));

-- Negative gamma (implementation-defined)
WITH colorOKLABToSRGB((0.591, 0.091, -0.0796), -1000) AS t
...
WITH colorOKLABToSRGB((0.5, 1., 1.)) AS t

SELECT tuple(round((t).1, 6), round((t).2, 6), round((t).3, 6));

-- Very large values
WITH colorOKLABToSRGB((1000., 1000000., 1000000.)) AS t
...
WITH colorOKLABToSRGB((1000., 1000000., 1000000.), materialize(0.)) AS t
