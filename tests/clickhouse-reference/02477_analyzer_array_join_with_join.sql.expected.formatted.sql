...
FROM
    test_table
ARRAY JOIN value_array AS value_array;
...
FROM
    (
        SELECT [5] AS id_array
    ) AS subquery
ARRAY JOIN id_array, [0] AS id
INNER JOIN test_table
    USING (id);
...
ARRAY JOIN id AS id_nested_array
