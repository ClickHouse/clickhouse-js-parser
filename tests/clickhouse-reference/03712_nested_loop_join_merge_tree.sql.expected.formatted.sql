...
SELECT
    32,
    'Payload_Dup',
    toDateTime('2024-01-01 00:10:00');
...
SELECT
    ...
    if(ProfileEvents['JoinBuildTableRowCount'] >= 10
    AND ProfileEvents['JoinBuildTableRowCount'] <= 1000000, 'OK', format('Fail: JoinBuildTableRowCount={}, query_id={}', ProfileEvents['JoinBuildTableRowCount'], query_id))
...
SELECT
    ...
    if(ProfileEvents['JoinBuildTableRowCount'] >= 1100000
    AND ProfileEvents['JoinBuildTableRowCount'] <= 2100000, 'OK', format('Fail: JoinBuildTableRowCount={}, query_id="{}"', ProfileEvents['JoinBuildTableRowCount'], query_id))
FROM `system`.query_log
WHERE type = 'QueryFinish'
    AND current_database = currentDatabase()
    AND event_time >= yesterday()
