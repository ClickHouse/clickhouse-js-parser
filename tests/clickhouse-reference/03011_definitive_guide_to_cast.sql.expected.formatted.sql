...
SELECT CAST(100000000000000000000. AS DateTime64(6, 'Europe/Amsterdam'));
...
SELECT CAST(100000000000000000000. AS DateTime64(9, 'Europe/Amsterdam')); -- { serverError DECIMAL_OVERFLOW }
...
FORMAT TSV; -- { serverError TOO_LARGE_STRING_SIZE }
...
SELECT 1 - CAST('1' AS String); -- { serverError ILLEGAL_TYPE_OF_ARGUMENT }
...
SELECT
    1 AS x,
    -x::String; -- { serverError ILLEGAL_TYPE_OF_ARGUMENT }
...
SELECT accurateCastOrDefault(-1, 'UInt64', CAST('0' AS UInt64));
...
FORMAT TSV; -- the parameter of the FixedString data type, the function returns FixedString(10)

SELECT toFixedString('Hello', 5 + 5)
FORMAT TSV; -- it can be a constant expression
...
SELECT toString(CAST('1710612085' AS DateTime), 'America/Los_Angeles');

SELECT toString(CAST('1710612085' AS DateTime));
...
SELECT toString(CAST('1710612085' AS DateTime), tz)
...
SELECT toUInt8OrNull(CAST('123' AS FixedString(3)));
