...
CREATE TABLE tab
(
    c0 FixedString(9) CODEC(DoubleDelta(1))
)
ENGINE = MergeTree()
ORDER BY tuple(); -- { serverError BAD_ARGUMENTS }
...
CREATE TABLE tab
(
    c0 LowCardinality(FixedString(9)) CODEC(DoubleDelta(2))
)
ENGINE = MergeTree()
ORDER BY tuple(); -- { serverError BAD_ARGUMENTS }
...
CREATE TABLE tab
(
    c0 LowCardinality(Nullable(Time)) CODEC(DoubleDelta(2))
)
ENGINE = MergeTree()
ORDER BY tuple(); -- { serverError BAD_ARGUMENTS }
