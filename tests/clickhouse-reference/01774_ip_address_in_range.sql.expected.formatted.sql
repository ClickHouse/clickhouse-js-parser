...
INSERT INTO test_data SELECT IPv4NumToString(IPv4CIDRToRange(IPv4StringToNum('255.255.255.255'), toUInt8(number)).1) || '/' || toString(number) AS cidr
FROM `system`.numbers
LIMIT 33;
...
SELECT sum(isIPAddressInRange('127.0.0.0', cidr)) = 1
FROM test_data;

SELECT sum(isIPAddressInRange('128.0.0.0', cidr)) = 2
FROM test_data;

SELECT sum(isIPAddressInRange('255.0.0.0', cidr)) = 9
FROM test_data;

SELECT sum(isIPAddressInRange('255.0.0.1', cidr)) = 9
FROM test_data;

SELECT sum(isIPAddressInRange('255.0.0.255', cidr)) = 9
FROM test_data;

SELECT sum(isIPAddressInRange('255.255.255.255', cidr)) = 33
FROM test_data;

SELECT sum(isIPAddressInRange('255.255.255.254', cidr)) = 32
FROM test_data;
