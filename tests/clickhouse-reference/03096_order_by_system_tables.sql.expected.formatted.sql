...
SELECT 'Table ' || name || ' has non-default sorting key: ' || sorting_key
FROM `system`.tables
WHERE database = 'system'
    AND engine = 'MergeTree'
    AND name NOT LIKE 'minio%'
    AND NOT arraySum(arrayMap((x -> position(name, x)), known_tables))
    AND sorting_key != default_sorting_key;