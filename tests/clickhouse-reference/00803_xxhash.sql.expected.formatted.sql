...
SELECT xxHash64(x) = toUInt64(9962287286179718960)
FROM (
        SELECT CAST(1 AS Enum8('a' = 1, 'b' = 2)) AS x
    );

SELECT xxHash64('') = toUInt64(9962287286179718960);

SELECT xxHash64('\0') = toUInt64(6482051057365497128);

SELECT xxHash64('\0\0\0') = toUInt64(13361037350151369407);
...
SELECT xxHash32(x) = toUInt32(949155633)
FROM (
        SELECT CAST(1 AS Enum8('a' = 1, 'b' = 2)) AS x
    );

SELECT xxHash32('') = toUInt32(949155633);

SELECT xxHash32('\0') = toUInt32(332955956);

SELECT xxHash32('\0\0\0') = toUInt32(2158931063);
