...
(
    id UInt64,
    msg String,
    INDEX id_msg msg TYPE text(tokenizer = sparseGrams) GRANULARITY 100000000
)
ENGINE = MergeTree()
ORDER BY tuple()
...
SELECT count()
FROM tab
WHERE hasAllTokens(msg, sparseGrams('click'));
...
SELECT sum(length(tokens(msg, 'sparseGrams')))
FROM tab;