...
INSERT INTO t_min_bytes_to_seek SELECT number
FROM numbers(10000);

SELECT count()
FROM t_min_bytes_to_seek
WHERE id IN (10, 1000, 5000, 9000)
SETTINGS merge_tree_min_bytes_for_seek = '0';
...
FROM t_min_bytes_to_seek
WHERE id IN (10, 1000, 5000, 9000)
SETTINGS merge_tree_min_rows_for_seek = '0';

SELECT count()
FROM t_min_bytes_to_seek
WHERE id IN (10, 1000, 5000, 9000)
SETTINGS merge_tree_min_rows_for_seek = '1000000000';
...
WHERE current_database = currentDatabase()
    AND query LIKE 'SELECT count() FROM t_min_bytes_to_seek%'
