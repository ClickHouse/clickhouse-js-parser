...
SELECT arrayElementOrNull(range(number), 2)
FROM `system`.numbers
LIMIT 3;

SELECT arrayElementOrNull(range(number), -1)
FROM `system`.numbers
LIMIT 3;

SELECT arrayElementOrNull(range(number), number)
FROM `system`.numbers
LIMIT 3;

SELECT arrayElementOrNull(range(number), 2 - number)
FROM `system`.numbers
LIMIT 3;
...
SELECT
    arrayElementOrNull([[1]], 1),
    arrayElementOrNull(materialize([[1]]), 1),
    arrayElementOrNull([[1]], materialize(1)),
    arrayElementOrNull(materialize([[1]]), materialize(1));

SELECT
    arrayElementOrNull([['Hello']], 1),
    arrayElementOrNull(materialize([['World']]), 1),
    arrayElementOrNull([['Hello']], materialize(1)),
...
SELECT
    arrayElementOrNull([1, 2], 3),
    arrayElementOrNull([1, NULL, 2], 4),
    arrayElementOrNull([('1', 1), ('2', 2)], -3);

SELECT
    groupArray(a) AS b,
    arrayElementOrNull(b, 1),
    arrayElementOrNull(b, 0)
FROM (
        SELECT (1, 2) AS a
    );
...
SELECT
    [toNullable(1)] AS x,
    arrayElementOrNull(x, toNullable(1)) AS y;

SELECT
    materialize([toNullable(1)]) AS x,
    arrayElementOrNull(x, toNullable(1)) AS y;

SELECT
    [toNullable(1)] AS x,
    arrayElementOrNull(x, materialize(toNullable(1))) AS y;
