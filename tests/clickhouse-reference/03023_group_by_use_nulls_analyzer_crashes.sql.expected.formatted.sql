...
SELECT tuple(array(number)) AS x
FROM numbers(10)
GROUP BY
    number,
    array(number)
...
SELECT tuple(number) AS x
FROM numbers(10)
GROUP BY GROUPING SETS ((number))
ORDER BY x ASC;

SELECT
    ignore(toFixedString('Lambda as function parameter', 28), toNullable(28), ignore(8)),
    sum(marks)
FROM `system`.parts
...
SELECT tuple(tuple(number)) AS x
FROM numbers(10)
WHERE toString(toUUID(tuple(number), NULL), x)
GROUP BY
    number,
    (toString(x), number)
WITH CUBE
...
FROM (
        SELECT
            ignore(ignore('Lambda as function parameter', 28, 28, 28, 28, 28, 28), 28),
            materialize('Lambda as function parameter'),
            28,
            28,
            'world',
            5
        FROM `system`.numbers
        WHERE ignore(materialize('Lambda as function parameter'), materialize(toLowCardinality(28)), 28, 28, 28, 28, toUInt128(28))
        LIMIT 2
    )
