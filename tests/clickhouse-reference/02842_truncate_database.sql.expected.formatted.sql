...
CREATE TABLE source_table_memory
(
    x UInt16
)
ENGINE = Memory();
...
INSERT INTO source_table_memory SELECT *
FROM `system`.numbers
LIMIT 10;

INSERT INTO source_table_log SELECT *
FROM `system`.numbers
LIMIT 10;

INSERT INTO source_table_tiny_log SELECT *
FROM `system`.numbers
LIMIT 10;

INSERT INTO source_table_stripe_log SELECT *
FROM `system`.numbers
LIMIT 10;

INSERT INTO source_table_merge_tree SELECT *
FROM `system`.numbers
LIMIT 10;

INSERT INTO source_table_dictionary;

-- create view based on the tables
CREATE VIEW dest_view_memory (x UInt64)
AS
SELECT *
FROM source_table_memory;

CREATE VIEW dest_view_log (x UInt64)
AS
SELECT *
FROM source_table_log;

CREATE VIEW dest_view_tiny_log (x UInt64)
AS
SELECT *
FROM source_table_tiny_log;

CREATE VIEW dest_view_stripe_log (x UInt64)
AS
SELECT *
FROM source_table_stripe_log;

CREATE VIEW dest_view_merge_tree (x UInt64)
AS
SELECT *
FROM source_table_merge_tree;
...
SELECT *
FROM dest_view_tiny_log
ORDER BY x ASC
LIMIT 1;

SELECT *
FROM dest_view_stripe_log
ORDER BY x ASC
LIMIT 1;

SELECT *
FROM dest_view_merge_tree
ORDER BY x ASC
LIMIT 1;
...
WHERE database = 'test_truncate_database'
    AND name = 'dest_dictionary';
