...
SELECT (
        ...
        SELECT arraySort(groupArray(CAST(lc AS String)))
        FROM (
                SELECT DISTINCT lc
                FROM distinct_lc_basic
            )
...
SELECT (
        ...
        SELECT arraySort(groupArray(lc))
        FROM (
                SELECT DISTINCT lc
                FROM distinct_lc_nullable
            )
...
SELECT (
        SELECT count()
        FROM (
                SELECT DISTINCT toString(intDiv(number, 7)) AS x
                FROM numbers(100000)
            )
    ) - (
        SELECT count()
        FROM (
                SELECT DISTINCT toLowCardinality(toString(intDiv(number, 7))) AS x
                FROM numbers(100000)
            )
    );
...
SELECT (
        ...
        SELECT arraySort(groupArray(CAST(x AS String)))
        FROM (
                SELECT DISTINCT toLowCardinality(toString(intDiv(number, 7))) AS x
                FROM numbers(100000)
            )
