...
SELECT c_dynamic IS DISTINCT FROM c_ipv4
FROM `03611_nscmp_tbl`
ORDER BY key ASC;

SELECT c_dynamic <=> c_ipv6
FROM `03611_nscmp_tbl`
ORDER BY key ASC;

SELECT c_dynamic IS DISTINCT FROM c_ipv6
FROM `03611_nscmp_tbl`
ORDER BY key ASC;
...
SELECT c_tuple <=> NULL AS tuple_vs_null
FROM `03611_nscmp_tbl`;

SELECT NULL <=> c_tuple AS tuple_vs_null
FROM `03611_nscmp_tbl`;

SELECT c_tuple IS DISTINCT FROM NULL AS tuple_vs_null_distinct
FROM `03611_nscmp_tbl`;

SELECT NULL IS DISTINCT FROM c_tuple AS tuple_vs_null_distinct
FROM `03611_nscmp_tbl`;
...
SELECT
    ...
    r.b
FROM
    `03611_t_nullsafe` AS l
INNER JOIN `03611_t_nullsafe` AS r
    ON l.a <=> r.b
ORDER BY
    lid ASC,
    rid ASC;
...
SELECT
    ...
    a,
    b,
    multiIf(a <=> b, 'null_safe_equal', a IS DISTINCT FROM b, 'null_safe_distinct', 'unexpected') AS comparison_result,
    multiIf(a <=> b, 'same', a IS DISTINCT FROM b, 'different', 'unknown') AS status
FROM `03611_t_nullsafe`
ORDER BY id ASC;
...
FROM `03611_t_nullsafe`
ORDER BY distinct_flag ASC;

SELECT
    1 <=> 1
    AND 1 IS DISTINCT FROM 1,
    1 <=> 1
...
GROUP BY 1
WITH TOTALS
QUALIFY ((2 <=> 2) / ((2 IS NOT NULL) IS NULL), *, *, materialize(toNullable(2)), materialize(2), materialize(materialize(2)), 2) <=> ((2 = *) * 2, *, *, 2, toNullable(2), 2, 2);
