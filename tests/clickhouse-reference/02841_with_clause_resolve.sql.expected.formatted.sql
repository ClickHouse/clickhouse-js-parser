...
-- Input
WITH 44100 AS sample_frequency,
...
WITH 44100 AS sample_frequency,
  ...
  level -> (clamp(level) * 32767 * master_volume)::Int16 AS output,
  ...
  -- Envelopes
  (time, offset, attack, hold, release) -> time < `offset` ? 0 : (time < `offset` + attack ? (time - `offset`) / attack : (time < `offset` + attack + hold ? 1 : (time < `offset` + attack + hold + release ? (`offset` + attack + hold + release - time) / release : 0))) AS envelope,
