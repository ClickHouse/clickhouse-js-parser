...
WITH materialize([1, 2, NULL]) AS arr
...
WITH map('foo', 1, 'bar', 2) AS m

SELECT
    m.keys,
    m.values;

WITH materialize(map('foo', 1, 'bar', 2)) AS m
...
SELECT arr.*; -- { serverError UNSUPPORTED_METHOD }
