...
FROM `system`.metrics
WHERE name LIKE '%VectorSimilarityIndexCacheBytes%';
...
SELECT
    ...
    value
FROM `system`.metrics
WHERE name LIKE '%VectorSimilarityIndexCacheBytes%'; -- ALTER TABLE ... DROP INDEX <vector index> and MERGE PARTS will also clear
