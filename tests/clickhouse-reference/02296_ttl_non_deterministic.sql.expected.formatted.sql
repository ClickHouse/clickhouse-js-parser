...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{database}/ttl1', '1')
ORDER BY A
TTL now() + toIntervalMonth(1); -- {serverError BAD_ARGUMENTS}
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{database}/ttl2', '1')
ORDER BY A;
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{database}/ttl3', '1')
ORDER BY A; -- {serverError BAD_ARGUMENTS}
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{database}/ttl4', '1')
ORDER BY A;