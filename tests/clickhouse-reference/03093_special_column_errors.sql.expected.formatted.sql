...
ORDER BY key; -- { serverError BAD_TYPE_OF_FIELD }
...
ENGINE = ReplacingMergeTree(is_deleted, is_deleted)
...
ENGINE = CollapsingMergeTree(not_existing)
ORDER BY key; -- { serverError NO_SUCH_COLUMN_IN_TABLE }
...
ENGINE = VersionedCollapsingMergeTree(sign, sign)
ORDER BY key; -- { serverError BAD_ARGUMENTS }
