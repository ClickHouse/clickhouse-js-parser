...
SELECT
    emptyArrayToSingle(emptyArrayString()),
    emptyArrayToSingle(emptyArrayDate()),
    emptyArrayToSingle(arrayFilter((x -> 0), [now('Asia/Istanbul')]));
...
SELECT
    ...
    emptyArrayToSingle(arrayMap((x -> toDate('2015-01-01') + x), range(number % 4)))
FROM `system`.numbers
LIMIT 10;