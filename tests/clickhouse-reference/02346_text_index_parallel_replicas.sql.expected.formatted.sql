...
(
    id UInt64,
    str String,
    INDEX idx_str str TYPE text(tokenizer = splitByNonAlpha) GRANULARITY 100000000
)
ENGINE = MergeTree()
ORDER BY id
PARTITION BY id;
...
INSERT INTO tab SELECT
    1,
    arrayStringConcat(arrayMap((x -> toString(number + x * 2)), range(5)), ' ')
...
WHERE (current_database = currentDatabase()
    ...
    AND query LIKE '%SELECT%tab%hasAnyTokens%'
