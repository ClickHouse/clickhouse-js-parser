...
FROM remote('127.1', `system`.one); -- { serverError FUNCTION_THROW_IF_VALUE_IS_NON_ZERO }

-- these are just in case
SELECT throwIf(dummy = 0)
FROM remote('127.{1,2}', `system`.one); -- { serverError FUNCTION_THROW_IF_VALUE_IS_NON_ZERO }
