...
SELECT n
FROM test1
WHERE toFloat32(n) = 7777.
SETTINGS
    max_rows_to_read = '2',
    parallel_replicas_index_analysis_only_on_coordinator = '0';

INSERT INTO test2 SELECT toString(number)
FROM numbers(10000);

SELECT s
FROM test2
WHERE toFloat64(s) = 7777.;

SELECT s
FROM test2
WHERE toFloat32(s) = 7777.;

INSERT INTO test3 SELECT toDecimal64(number, 3)
FROM numbers(10000);

SELECT d
FROM test3
WHERE toFloat64(d) = 7777.
SETTINGS
    max_rows_to_read = '2',
    parallel_replicas_index_analysis_only_on_coordinator = '0';
