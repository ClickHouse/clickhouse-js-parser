...
SELECT (number AS n, n + 1, n + 2, n + 3) IN (1, 2, 3, 4)
FROM `system`.numbers
LIMIT 3;

SELECT (number AS n, n + 1, n + 2, n + 3, n - 1) IN (1, 2, 3, 4, 0)
FROM `system`.numbers
LIMIT 3;

SELECT (number AS n, n + 1, toString(n + 2), n + 3, n - 1) IN (1, 2, '3', 4, 0)
FROM `system`.numbers
LIMIT 3;
...
WHERE (number, tuple) IN ((1, (2, 3)), (4, (5, 6)));
