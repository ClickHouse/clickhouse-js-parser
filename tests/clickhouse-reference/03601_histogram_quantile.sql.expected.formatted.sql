...
FROM (
        SELECT
            arrayJoin(arrayZip([0., 0.5, 1., inf], [0., 10., 11., 12.])) AS args,
            number
        FROM numbers(10)
    )
GROUP BY number
...
FROM (
        SELECT arrayJoin(arrayZip([0., 0.5, 1.], [0., 10., 11.])) AS args
    );

SELECT quantilePrometheusHistogram(0.5)(inf, 10.); -- return NaN if less than 2 buckets

SELECT quantilePrometheusHistogram(0.2)((args).1, (args).2) -- interpolate between minimum bucket upper bound and 0
FROM (
        SELECT arrayJoin(arrayZip([0.5, 1., 2., inf], [5., 10., 13., 15.])) AS args
    );
...
FROM (
        SELECT arrayJoin(arrayZip([-0.5, 0., 1., inf], [5., 10., 13., 15.])) AS args
    );
