...
SELECT regexp_replace(identity('abc123'), '^abc', '');

EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpAll(identity('abc123'), '^abc', '');

-- Ends with unescaped $ (should rewrite)
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT regexp_replace(identity('abc123'), '123$', '');

EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpAll(identity('abc123'), '123$', '');
...
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpAll(identity('abc123'), 'a^bc', '');

-- Pattern with $ not at end (should NOT rewrite)
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpAll(identity('abc123'), '123$abc', '');

-- Pattern with alternatives (should NOT rewrite)
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpAll(identity('abc123'), '^123|456$', '');
...
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpOne(identity('abc123'), '^abc123$', '\\0');

-- Only \1, with one capture group (should rewrite)
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpOne(identity('abc123'), '^(abc)$', '\\1');
...
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpOne(identity('abc123'), '^abc', '\\0');
...
-- Pattern not full (should NOT rewrite)
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpOne(identity('abc123'), '^ab|c$', '\\0');

-- \0 with extra characters (should NOT rewrite)
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpOne(identity('abc123'), '^abc123$', 'pre\\0post');

-- \1 with two capture groups (should rewrite — only \1 used)
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpOne(identity('abc123'), '^(a)(b)$', '\\1');

-- \2 used (should NOT rewrite)
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpOne(identity('abc123'), '^(a)(b)$', '\\2');

-- Mixed content in replacement (should NOT rewrite)
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpOne(identity('abc123'), '^(abc)$', 'X\\1Y');
...
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT extract(identity('abc123'), '^.*(123)');

-- Ends with unescaped .*$ (should strip suffix)
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT extract(identity('abc123'), '(abc).*$');
...
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT extract(identity('abc123'), '^.*$');
...
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT extract(identity('abc123abc456'), '^.*?(abc.*)');
...
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpAll(identity('abc'), '^(abc)', '\\1');

-- Rule 2 + 3: replaceRegexpOne -> extract -> simplified extract
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT replaceRegexpOne(identity('abc'), '^.*(abc).*$', '\\1');
...
-- ClickBench Q28
EXPLAIN QUERY TREE dump_tree = '0', dump_ast = '1'
SELECT REGEXP_REPLACE(identity('some referer'), '^https?://(?:www\\.)?([^/]+)/.*$', '\\1');