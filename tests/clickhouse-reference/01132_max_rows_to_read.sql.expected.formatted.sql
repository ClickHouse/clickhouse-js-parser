...
SELECT count()
FROM numbers(21); -- { serverError TOO_MANY_ROWS }
...
INSERT INTO row_limits_fail_fast SELECT
    number,
    toString(number)
FROM numbers(10000);
...
WHERE key < 500000; -- { serverError TOO_MANY_ROWS }
...
SELECT count()
FROM row_limits_fail_fast
WHERE key < 100000; -- { serverError TOO_MANY_ROWS }
