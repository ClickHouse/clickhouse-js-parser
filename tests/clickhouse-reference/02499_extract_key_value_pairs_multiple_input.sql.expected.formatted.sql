...
WITH extractKeyValuePairs('name:neymar, age:31 team:psg,nationality:brazil') AS s_map,

CAST(arrayMap((x -> (x, s_map[x])), arraySort(mapKeys(s_map))), 'Map(String,String)') AS x
...
WITH extractKeyValuePairs('1name:neymar, 4ge:31 _team:_psg,$nationality:@brazil') AS s_map,
...
-- expected output: {'#':'#','$':'$','@':'@','_':'_'}
WITH extractKeyValuePairs('_:_, @:@ #:#,$:$') AS s_map,
...
WITH extractKeyValuePairs('name:ney!mar, age:3! t&am:@psg,nationality:br4z!l') AS s_map,
...
WITH extractKeyValuePairs('currency:\\$USD, amount\\z:$5\\h') AS s_map,
...
WITH extractKeyValuePairsWithEscaping('valid_key:valid_value key:invalid_escape_sequence\\', ':', ' ', '"') AS s_map,
...
WITH extractKeyValuePairs('name:"neymar", "age":31 "team":"psg"') AS s_map,
...
WITH extractKeyValuePairs('name:"", age: , nationality:') AS s_map,
...
WITH extractKeyValuePairs('"":abc, :def') AS s_map,
...
WITH extractKeyValuePairs('name:neymar;age:31;team:psg;random_key:value_with_comma,still_part_of_value:still_part_of_value;anotherkey:anothervalue', ':', ';') AS s_map,
...
WITH extractKeyValuePairs('name:neymar;age:31;team:psg;nationality:brazil,last_key:last_value', ':', ';,') AS s_map,
...
WITH extractKeyValuePairs('not_important', ':', ',:', '''') AS s_map,

CAST(arrayMap((x -> (x, s_map[x])), arraySort(mapKeys(s_map))), 'Map(String,String)') AS x

SELECT x; -- {serverError BAD_ARGUMENTS}
...
WITH extractKeyValuePairs([1, 2]) AS s_map,

CAST(arrayMap((x -> (x, s_map[x])), arraySort(mapKeys(s_map))), 'Map(String,String)') AS x

SELECT x; -- {serverError ILLEGAL_TYPE_OF_ARGUMENT}
...
WITH extractKeyValuePairs('not_important', ':', '123456789', '''') AS s_map,
...
WITH extractKeyValuePairs() AS s_map,

CAST(arrayMap((x -> (x, s_map[x])), arraySort(mapKeys(s_map))), 'Map(String,String)') AS x

SELECT x; -- {serverError NUMBER_OF_ARGUMENTS_DOESNT_MATCH}

-- should fail because one extra argument / non existent has been provided
WITH extractKeyValuePairs('a', ':', ',', '"', 'accept', '') AS s_map,
...
WITH extractKeyValuePairs('key1:value1,key2:value2') AS s_map,

CAST(arrayMap((x -> (x, s_map[x])), arraySort(mapKeys(s_map))), 'Map(String,String)') AS x

SELECT x; -- {serverError LIMIT_EXCEEDED}
...
WITH extractKeyValuePairs('not_important', ':', '12345678', '''') AS s_map,
...
WITH extractKeyValuePairs('formula=1+2=3 argument1=1 argument2=2 result=3, char="=" char2== string="foo=bar"', '=') AS s_map,
...
WITH extractKeyValuePairs('{"a":"1", "b":"2"}', ':', '{, }') AS s_map,
...
WITH sTr_tO_mAp('name:neymar, age:31 team:psg,nationality:brazil') AS s_map,
...
WITH mapFromString('name:neymar, age:31 team:psg,nationality:brazil') AS s_map,
...
WITH extractKeyValuePairs('key:"#123"junk", second_key:0') AS s_map,
...
WITH extractKeyValuePairs('name"abc":5', ':', ' ,;', '"', 'accept') AS s_map,
...
WITH extractKeyValuePairs('name"abc":5', ':', ' ,;', '"', 'invalid') AS s_map,
...
WITH extractKeyValuePairs('name"abc":5', ':', ' ,;', '"', 'promote') AS s_map,
...
WITH extractKeyValuePairs('key:val"abc', ':', ' ,;', '"', 'accept') AS s_map,
...
-- unexpected_quoting_character_strategy=invalid, empty return
WITH extractKeyValuePairs('key:val"abc', ':', ' ,;', '"', 'invalid') AS s_map,
...
WITH extractKeyValuePairs('key:val"abc', ':', ' ,;', '"', 'promote') AS s_map,
...
WITH extractKeyValuePairs('key:"quoted_value"junk,second_key:0') AS s_map,
