...
SELECT arrayFold(((acc, x) -> acc + x), range(number), number)
FROM `system`.numbers
LIMIT 5;

SELECT arrayFold(((acc, x) -> arrayPushFront(acc, x)), range(number), emptyArrayUInt64())
FROM `system`.numbers
LIMIT 5;

SELECT arrayFold(((acc, x) -> x % 2 ? arrayPushFront(acc, x) : arrayPushBack(acc, x)), range(number), emptyArrayUInt64())
FROM `system`.numbers
LIMIT 5;
...
SELECT
    line,
    patterns,
    arrayFold(((acc, pat) -> position(line, pat)), patterns, CAST('0' AS UInt64))
FROM tab
ORDER BY line ASC;
...
SELECT
    ...
    arrayFold(((acc, str) -> position(line, str)), splitByNonAlpha(line), CAST('0' AS UInt64))
FROM tab;
