...
INSERT INTO bloom_filter_types_test SELECT
    ...
    toFloat32(number) AS f32,
    toFloat64(number) AS f64,
    toDate(number, 'Asia/Istanbul') AS date,
    toDateTime(number, 'Asia/Istanbul') AS date_time,
    toString(number) AS str,
    toFixedString(toString(number), 5) AS fixed_string,
    toDateTime64(number, 3, 'Asia/Istanbul') AS dt64
FROM `system`.numbers
LIMIT 100;
...
INSERT INTO bloom_filter_array_types_test SELECT
    ...
    groupArray(toDateTime64(number, 3, 'Asia/Istanbul')) AS dt64
FROM (
        SELECT number
        FROM `system`.numbers
        LIMIT 15
