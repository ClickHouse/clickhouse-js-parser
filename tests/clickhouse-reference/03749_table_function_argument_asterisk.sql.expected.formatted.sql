SELECT *
FROM remote(*, '127.{1,2}', view(    SELECT 2)); -- { serverError BAD_ARGUMENTS }

SELECT *
FROM remote(*, view(    SELECT 2)); -- { serverError BAD_ARGUMENTS }

SELECT *
FROM remote(*, '127.{1,2}', view(    SELECT toLowCardinality(2))); -- { serverError BAD_ARGUMENTS }

SELECT *
FROM remote(*, '127.{1,2}', view(    SELECT 1
    FROM numbers(1)
    GROUP BY toLowCardinality(2))); -- { serverError BAD_ARGUMENTS }
