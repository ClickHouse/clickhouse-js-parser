...
CREATE TABLE dict_in_01023.input
(
    key UInt64,
    val UInt64
)
...
ENGINE = Distributed(test_shard_localhost, currentDatabase(), buffer_, key);
...
ENGINE = Buffer(currentDatabase(), dist_out, 1, 0, 0, 0, 0, 0, 0);

CREATE TABLE dist_out
(
    key UInt64
)
ENGINE = Distributed(test_shard_localhost, currentDatabase(), null_, key);
...
SELECT
    key,
    dictGetUInt64('dict_in_01023.dict', 'val', key) AS val
FROM dist_out;
